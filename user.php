...
$ui_arr = array('register', 'login', 'profile', 'order_list', 'order_detail', 'address_list', 'collection_list',
    'message_list', 'supply_list','delivery','goods_sell', 'complete','not_sale','tag_list', 'get_password', 'reset_password', 'booking_list', 'add_booking', 'account_raply', 'account_transfer', 'exp_value_buy', 'exp_value_sell', 'exp_value_exchange',
    'account_deposit', 'account_log', 'aaccount_log', 'account_detail', 'act_account', 'pay', 'default', 'bonus', 'group_buy', 'group_buy_detail', 'affiliate', 'comment_list', 'validate_email', 'track_packages', 'transform_points', 'qpassword_name', 'get_passwd_question', 'check_answer');
...
/* 显示供货列表 */ elseif ($action == 'supply_list') {
    include_once(ROOT_PATH . 'includes/lib_clips.php');

    $page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;

    $sql = "SELECT COUNT(*) FROM " . $ecs->table('supply_apply') .
            " WHERE parent_id = 0 AND user_id = '$user_id' AND user_name = '" . $_SESSION['user_name'] . "' AND order_id=0";
    $record_count = $db->getOne($sql);
    $act = array('act' => $action);
    ...
    $smarty->assign('supply_list', get_supply_list($user_id, $_SESSION['user_name'], $pager['size'], $pager['start'], 0));
    $smarty->assign('pager', $pager);
    $smarty->assign('order_info', $order_info);
    $smarty->display('user_clips.dwt');
}
...
elseif ($action == 'delivery') {
    ...
    $inv_no = isset($_POST['inv_no']) ? intval($_POST['inv_no']) : 0;

    if ($msg_id) {
        $sql="UPDATE ".$ecs->table('supply_apply')." set msg_status =2,msg_inv_no=$inv_no WHERE msg_id=".$msg_id;
        $db->query($sql);
        ecs_header("Location: user.php?act=supply_list\n");
        exit;
    } else {
        $err->show($_LANG['goods_sell'], 'user.php?act=supply_list');
    }
}
/* 我的同城销售 */ elseif ($action == 'goods_sell') {
    include_once(ROOT_PATH . 'includes/lib_clips.php');

    $page = isset($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;

    $sql = "SELECT COUNT(*) FROM " . $ecs->table('goods') .
            " WHERE is_local = 1 AND user_id = '$user_id'";
    $record_count = $db->getOne($sql);
    $act = array('act' => $action);

    $pager = get_pager('user.php', $act, $record_count, $page, 5);

    $smarty->assign('local_list', get_local_list($user_id, $pager['size'], $pager['start']));
    $smarty->assign('pager', $pager);
    $smarty->display('user_clips.dwt');
}/*已交货*/
elseif ($action == 'complete') {
    ...
    if ($goods_id) {
        $sql="UPDATE ".$ecs->table('goods')." set local_status =1 WHERE goods_id=".$goods_id;
        $db->query($sql);
        ecs_header("Location: user.php?act=goods_sell\n");
        exit;
    } else {
        $err->show($_LANG['goods_sell'], 'user.php?act=order_list');
    }
}
/*申请下架*/
elseif ($action == 'not_sale') {
    include_once(ROOT_PATH . 'includes/lib_transaction.php');
    include_once(ROOT_PATH . 'includes/lib_order.php');
    ...
    if ($goods_id) {
        $sql="UPDATE ".$ecs->table('goods')." set is_on_sale =0 WHERE goods_id=".$goods_id;
        $db->query($sql);
        ecs_header("Location: user.php?act=goods_sell\n");
        exit;
    } else {
        $err->show($_LANG['goods_sell'], 'user.php?act=goods_sell');
    }
}
...
elseif ($action == 'act_add_supply') {
    ...
    if (add_supply($message)) {
        show_message($_LANG['add_supply_success'], $_LANG['supply_list_lnk'], 'user.php?act=supply_list', 'info');
    } else {
        $err->show($_LANG['supply_list_lnk'], 'user.php?act=supply_list');
    }
}
...
/* 会员退款申请界面 */ elseif ($action == 'account_raply') {
    $smarty->display('user_transaction.dwt');
} elseif ($action == 'account_transfer') {
    $smarty->display('user_transaction.dwt');
} elseif ($action == 'exp_value_buy') {
    include_once(ROOT_PATH . 'includes/lib_clips.php');
    $smarty->assign('payment', get_online_payment_list(false));
    $smarty->assign('exp_buy', $_CFG['exp_buy']);
    $smarty->display('user_transaction.dwt');
} elseif ($action == 'exp_value_sell') {
    $smarty->assign('exp_sell', $_CFG['exp_sell']);
    $smarty->display('user_transaction.dwt');
}
elseif ($action == 'exp_value_exchange') {
     include_once(ROOT_PATH . 'includes/lib_clips.php');
     $smarty->assign('exp_exchange', $_CFG['exp_exchange']);
    $smarty->assign('payment', get_online_payment_list(false));
    $smarty->assign('exp_exchange', $_CFG['exp_exchange']);
    $smarty->display('user_transaction.dwt');
}
...
/* 会员充值和提现申请记录以及转让经验值 */ elseif ($action == 'account_log') {
    include_once(ROOT_PATH . 'includes/lib_clips.php');
...
/* 会员购买和卖出经验值 */ elseif ($action == 'aaccount_log') {
    include_once(ROOT_PATH . 'includes/lib_clips.php');
    ...
    $sql = "SELECT COUNT(*) FROM " . $ecs->table('user_account') .
            " WHERE user_id = '$user_id'" .
            " AND process_type " . db_create_in(array(SURPLUS_TRANSFER, SURPLUS_BUY,SURPLUS_EXCHANGE));
    $record_count = $db->getOne($sql);
    ...
    //获取剩余余额
    $exp_amount = get_user_exp($user_id);
    if (empty($exp_amount)) {
        $exp_amount = 0;
    }

    //获取余额记录
    $account_log = get_aaccount_log($user_id, $pager['size'], $pager['start']);

    //模板赋值
    $smarty->assign('surplus_amount', $exp_amount);
    $smarty->assign('account_log', $account_log);
    $smarty->assign('pager', $pager);
    $smarty->display('user_transaction.dwt');
}
/* 对会员余额申请的处理 */ elseif ($action == 'act_account') {
    ...
    if ($surplus['process_type'] == 1) {
        ...
        $exp_value = get_user_exp($user_id);
        if ($amount > $exp_value) {
            $content = $_LANG['surplus_exp_error'];
            show_message($content, $_LANG['back_page_up'], '', 'info');
        }
        $sql = "select user_id from " . $ecs->table('users') . " where user_name = '" . $username . "'";
        $userId = $db->getOne($sql);
        if (!$userId) {
            $content = $_LANG['transfer_no_user'];
            show_message($content, $_LANG['back_page_up'], '', 'info');
        } elseif ($userId == $user_id) {
            $content = $_LANG['transfer_to_self'];
            show_message($content, $_LANG['back_page_up'], '', 'info');
        }
        $surplus['to_id'] = $userId;
    ...
    } elseif ($surplus['process_type'] == 4) {//卖经验值
        $exp_value = get_user_exp($user_id);
        if ($amount > $exp_value) {
            $content = $_LANG['surplus_exp_error'];
            show_message($content, $_LANG['back_page_up'], '', 'info');
        ...
        $amount = '-' . $amount;
        $surplus['payment'] = '';
        $surplus['process_type'] = "2";
        $surplus['rec_id'] = insert_user_account($surplus, $amount);

        /* 如果成功提交 */
        if ($surplus['rec_id'] > 0) {
            $content = $_LANG['surplus_appl_submit'];
            show_message($content, $_LANG['back_account_log'], 'user.php?act=aaccount_log', 'info');
        } else {
            $content = $_LANG['process_false'];
            show_message($content, $_LANG['back_page_up'], '', 'info');
        }
    } elseif ($surplus['process_type'] == 3) {//买经验值
        ...
        $order['surplus_amount'] = $amount * $_CFG['exp_buy'];

        //计算支付手续费用
        $payment_info['pay_fee'] = pay_fee($surplus['payment_id'], $order['surplus_amount'], 0);

        //计算此次预付款需要支付的总金额
        $order['order_amount'] = $amount * $_CFG['exp_buy'] + $payment_info['pay_fee'];
        ...
        $smarty->assign('amount', price_format($amount * $_CFG['exp_buy'], false));
        $smarty->assign('order', $order);
        $smarty->display('user_transaction.dwt');
    }
    elseif ($surplus['process_type'] == 5) {//经验值兑换金币
          $exp_value = get_user_exp($user_id);
        if ($amount > $exp_value) {
            $content = $_LANG['surplus_exp_error'];
            show_message($content, $_LANG['back_page_up'], '', 'info');
        ...
        $order['order_sn'] = $surplus['rec_id'];
        $order['user_name'] = $_SESSION['user_name'];
        $order['surplus_amount'] = abs($amount )* $_CFG['exp_buy'];

        //计算支付手续费用
        $payment_info['pay_fee'] = pay_fee($surplus['payment_id'], $order['surplus_amount'], 0);

        //计算此次预付款需要支付的总金额
        $order['order_amount'] = abs($amount) * $_CFG['exp_exchange'] + $payment_info['pay_fee'];
        ...
        $smarty->assign('payment', $payment_info);
        $smarty->assign('pay_fee', price_format($payment_info['pay_fee'], false));
        $smarty->assign('amount', price_format(abs($amount) * $_CFG['exp_exchange'], false));
        $smarty->assign('order', $order);
        $smarty->display('user_transaction.dwt');
    }
...
/* 删除会员余额 */ elseif ($action == 'cancel') {
    ...
    $sql = "SELECT process_type " .
            "FROM " . $GLOBALS['ecs']->table('user_account') .
            " WHERE id ='$id'";
    ...
    if ($result) {
        if ($type == 0 || $type == 1) {
            ...
            ecs_header("Location: user.php?act=aaccount_log\n");
        }
        exit;
    }
}
...
/* 编辑使用余额支付的处理 */ elseif ($action == 'act_edit_surplus') {
    ...
    log_account_change($user['user_id'], (-1) * $surplus, 0, 0, 0, 0, 0, $change_desc);
...
/* ajax 发送验证邮件 */ elseif ($action == 'send_hash_mail') {
    ...
    switch ($rule_key) {
        case TO_P :
            ...
            log_account_change($row['user_id'], 0, 0, 0, 0, 0, $result_points, $_LANG['transform_points'], ACT_OTHER);
        ...
        case TO_R :
            $result_points = floor($num * $to / $from);
            $user->set_points($row['user_name'], array($bbs_key => 0 - $num)); //调整论坛积分
            log_account_change($row['user_id'], 0, 0, 0, 0, $result_points, 0, $_LANG['transform_points'], ACT_OTHER);
        ...
        case FROM_P :
            $result_points = floor($num * $to / $from);
            log_account_change($row['user_id'], 0, 0, 0, 0, 0, 0 - $num, $_LANG['transform_points'], ACT_OTHER); //调整商城积分
        ...
        case FROM_R :
            $result_points = floor($num * $to / $from);
            log_account_change($row['user_id'], 0, 0, 0, 0, 0 - $num, 0, $_LANG['transform_points'], ACT_OTHER); //调整商城积分
            $user->set_points($row['user_name'], array($bbs_key => $result_points)); //调整论坛积分

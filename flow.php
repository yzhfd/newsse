...
//    elseif (check_local_cart($goods->goods_id)) {
//        $result['error'] = 1;
//        $result['message'] = $_LANG['local_is_alone'];
//    }
    /* 更新：购物车 */ else {
        // 更新：添加到购物车
    ...
    /* 如果使用金币，取得用户金币 */
    if ((!isset($_CFG['use_money']) || $_CFG['use_money'] == '1')
            && $_SESSION['user_id'] > 0
            && $user_info['exp_value'] > 0) {
        // 能使用金币
        $smarty->assign('allow_use_money', 1);
        $smarty->assign('your_money', $user_info['virtual_money']);
    }
    ...
    $json = new JSON();
    die($json->encode($result));
} elseif ($_REQUEST['step'] == 'change_money') {
    /* ------------------------------------------------------ */
    //-- 改变金币
    /* ------------------------------------------------------ */
    include_once('includes/cls_json.php');
    ...
    if ($user_info['virtual_money'] < $money) {
        $result['error'] = $_LANG['money_not_enough'];
    } else {
        /* 取得购物类型 */
        $flow_type = isset($_SESSION['flow_type']) ? intval($_SESSION['flow_type']) : CART_GENERAL_GOODS;
...
/* ------------------------------------------------------ */ elseif ($_REQUEST['step'] == 'done') {
    ...
    if ($user_id > 0) {
        ...
        $order['money'] = min($order['money'], $user_info['virtual_money']);
        if ($order['money'] < 0) {
            $order['money'] = 0;
        }
   ...
   $sql = "SELECT min(flow_no) FROM " . $GLOBALS['ecs']->table('cart') . " WHERE session_id ='" . SESS_ID."'";
    ...
    $sql = "INSERT INTO " . $ecs->table('order_goods') . "( " .
            ...
            "goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, goods_attr_id" .
            " FROM " . $ecs->table('cart') .
            " WHERE session_id = '" . SESS_ID . "' AND rec_type = '$flow_type' AND flow_no='$flow_no'";
    $db->query($sql);
    /* 修改拍卖活动状态 */
    ...
    if ($order['user_id'] > 0 && $order['surplus'] > 0) {
        log_account_change($order['user_id'], $order['surplus'] * (-1), 0, 0, 0, 0, 0, sprintf($_LANG['pay_order'], $order['order_sn']));
    }
    if ($order['user_id'] > 0 && $order['money'] > 0) {
        log_account_change($order['user_id'], 0, 0, $order['money'] * (-1), 0, 0, 0, sprintf($_LANG['pay_order'], $order['order_sn']));
    }
    if ($order['user_id'] > 0 && $order['integral'] > 0) {
        log_account_change($order['user_id'], 0, 0, 0, 0, 0, $order['integral'] * (-1), sprintf($_LANG['pay_order'], $order['order_sn']));
    }
            ...
            if (virtual_goods_ship($virtual_goods, $msg, $order['order_sn'], true)) {
                ...
                if ($db->getOne($sql) <= 0) {
                    ...
                    if ($order['user_id'] > 0) {
                        ...
                        log_account_change($order['user_id'], 0, 0, 0, 0, intval($integral['rank_points']), intval($integral['custom_points']), sprintf($_LANG['order_gift_integral'], $order['order_sn']));
...
/* 把优惠活动加入购物车 */ elseif ($_REQUEST['step'] == 'add_favourable') {
    /* 取得优惠活动信息 */
...
/* 验证红包序列号 */ elseif ($_REQUEST['step'] == 'validate_bonus') {
...
//    var_dump($cart_goods['goods_list']);die(0);

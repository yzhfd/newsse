...
elseif ($_REQUEST['act'] == 'delivery_cancel_ship')
{
    ...
    if ($order['order_status'] == SS_SHIPPED_ING)
    {
        ...
        if ($order['user_id'] > 0)
        {
            ...
            /* 计算并退回积分 */
            $integral = integral_to_give($order);
            log_account_change($order['user_id'], 0, 0,0,0, (-1) * intval($integral['rank_points']), (-1) * intval($integral['custom_points']), sprintf($_LANG['return_order_gift_integral'], $order['order_sn']));
...
elseif ($_REQUEST['act'] == 'step_post')
{
    ...
    elseif ('money' == $step)
    {
        ...
        if ($old_order['user_id'] > 0)
        {
            ...
            {
                log_account_change($user['user_id'], $user_money_change, 0, 0,0,0, 0, sprintf($_LANG['change_use_surplus'], $old_order['order_sn']));
            }

            $pay_points_change = $old_order['integral'] - $order['integral'];
            if ($pay_points_change != 0)
            {
                log_account_change($user['user_id'], 0, 0, 0,0,0, $pay_points_change, sprintf($_LANG['change_use_integral'], $old_order['order_sn']));
            }
...
function return_user_surplus_integral_bonus($order)
{
    ...
    if ($order['user_id'] > 0 && $order['surplus'] > 0)
    {
        ...
        log_account_change($order['user_id'], $surplus, 0,0,0, 0, 0, sprintf($GLOBALS['_LANG']['return_order_surplus'], $order['order_sn']));
        $GLOBALS['db']->query("UPDATE ". $GLOBALS['ecs']->table('order_info') . " SET `order_amount` = '0' WHERE `order_id` =". $order['order_id']);
    }

    if ($order['user_id'] > 0 && $order['integral'] > 0)
    {
        log_account_change($order['user_id'], 0, 0, 0,0,0, $order['integral'], sprintf($GLOBALS['_LANG']['return_order_integral'], $order['order_sn']));
    }

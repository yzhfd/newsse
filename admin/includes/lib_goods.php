 ...
 * @return  array   会员价格列表 user_rank => user_price
 */
function get_member_exp_list($goods_id)
{
    /* 取得会员价格 */
    $price_list = array();
    $sql = "SELECT user_rank, user_exp FROM " .
           $GLOBALS['ecs']->table('member_price') .
           " WHERE goods_id = '$goods_id'";
    $res = $GLOBALS['db']->query($sql);
    while ($row = $GLOBALS['db']->fetchRow($res))
 ...
 * @return  void
 */
function handle_member_price($goods_id, $rank_list, $price_list,$exp_list)
{
    /* 循环处理每个会员等级 */
    foreach ($rank_list AS $key => $rank)
    {
        ...
        if ($GLOBALS['db']->getOne($sql) > 0)
        {
            ...
            else
            {
                $sql = "UPDATE " . $GLOBALS['ecs']->table('member_price') .
                       " SET user_price = '$price',user_exp='$exp_list[$key]' " .
                       "WHERE goods_id = '$goods_id' " .
        ...
        else
        {
            ...
            {
                $sql = "INSERT INTO " . $GLOBALS['ecs']->table('member_price') . " (goods_id, user_rank, user_price,user_exp) " .
                       "VALUES ('$goods_id', '$rank', '$price','$exp_list[$key]')";
            }
        }

...
if ($_REQUEST['act'] == 'list')
{
    ...
    if (empty($_REQUEST['account_type']) || !in_array($_REQUEST['account_type'],
        array('user_money', 'frozen_money', 'rank_points', 'pay_points','virtual_money','exp_value')))
    {
        $account_type = '';
    }
    else
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    $virtual_money   = floatval($_POST['add_sub_virtual_money']) * abs(floatval($_POST['virtual_money']));
    $exp_value   = floatval($_POST['add_sub_exp_value']) * abs(floatval($_POST['exp_value']));
    ...
    if ($user_money == 0 && $frozen_money == 0 && $rank_points == 0 && $pay_points == 0&&$virtual_money&&$exp_value)
    {
        sys_msg($_LANG['no_account_change']);
    }

    /* 保存 */
    log_account_change($user_id, $user_money, $frozen_money, $virtual_money,$exp_value,$rank_points, $pay_points, $change_desc, ACT_ADJUSTING);
...
function get_accountlist($user_id, $account_type = '')
{
    ...
    if (in_array($account_type, array('user_money', 'frozen_money','virtual_money','exp_value', 'rank_points', 'pay_points')))
    {

...
elseif ($_REQUEST['act'] == 'settle_money')
{
    ...
    if (isset($_POST['unfreeze']))
    {
        ...
        log_account_change($auction['last_bid']['bid_user'], $auction['deposit'],
            (-1) * $auction['deposit'], 0,0,0, 0, sprintf($_LANG['unfreeze_auction_deposit'], $auction['act_name']));
    }
    else
    {
        /* 扣除 */
        log_account_change($auction['last_bid']['bid_user'], 0,
            (-1) * $auction['deposit'],0,0, 0, 0, sprintf($_LANG['deduct_auction_deposit'], $auction['act_name']));
    }

...
/* ------------------------------------------------------ */ elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy') {
    ...
    /*选择会员为供货商*/
    $suppliers_users=  get_users();
    $smarty->assign('suppliers_users',$suppliers_users);
    ...
    if (!$is_add) {
        $smarty->assign('member_price_list', get_member_price_list($_REQUEST['goods_id']));
         $smarty->assign('member_exp_list', get_member_exp_list($_REQUEST['goods_id']));
    }
    ...
    $sql = "SELECT a.region_id, r.region_name " .
            "FROM " . $ecs->table('goods_region') . " AS a, " . $ecs->table('region') . " AS r " .
            "WHERE r.region_id=a.region_id AND a.goods_id='$_REQUEST[goods_id]'";
    $res = $db->query($sql);
    ...
    /*    $sql = "SELECT region_name ".
      ...
      "WHERE region_id =".$goods['local_area'];
      $local_area = $db->getOne($sql); */
...
/* ------------------------------------------------------ */ elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
    ...
    $goods_virtual = isset($_POST['goods_virtual']) ? floatval($_POST['goods_virtual']) : '0.00';
    $goods_exp = isset($_POST['goods_exp']) ? intval($_POST['goods_exp']) : '0';
    $goods_newer = isset($_POST['goods_newer']) ? trim($_POST['goods_newer']) : '';
     $goods_standard = isset($_POST['goods_standard']) ? trim($_POST['goods_standard']) : '';
    $goods_flaw = isset($_POST['goods_flaw']) ? trim($_POST['goods_flaw']) : '';
    $is_local = isset($_POST['is_local']) ? intval($_POST['is_local']) : '0';
//    $local_area = isset($_POST['local_area']) ? trim($_POST['local_area']) : '';
    $local_area=0;
    $local_des = isset($_POST['local_des']) ? trim($_POST['local_des']) : '';
    ...
    if ($is_insert) {
        if ($code == '') {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    ...
                    "seller_note, goods_weight, goods_number, warn_number, integral,goods_virtual,goods_exp,goods_newer,goods_standard,goods_flaw,is_local,local_area,local_des, give_integral, is_best, is_new, is_hot, " .
                    ...
                    " '$warn_number', '$_POST[integral]', '$goods_virtual','$goods_exp','$goods_newer','$goods_standard',$goods_flaw','$is_local','1','$local_des','$give_integral', '$is_best', '$is_new', '$is_hot', '$is_on_sale', '$is_alone_sale', $is_shipping, " .
                    " '$_POST[goods_desc]', '" . gmtime() . "', '" . gmtime() . "', '$goods_type', '$rank_integral', '$suppliers_id','$user_id')";
        } else {
            $sql = "INSERT INTO " . $ecs->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                    "cat_id, brand_id, shop_price, market_price, is_promote, promote_price, " .
                    "promote_start_date, promote_end_date, goods_img, goods_thumb, original_img, keywords, goods_brief, " .
                    "seller_note, goods_weight, goods_number, warn_number, integral, goods_virtual,goods_exp,goods_newer,goods_standard,goods_flaw,is_local,local_area,local_des,give_integral, is_best, is_new, is_hot, is_real, " .
                    "is_on_sale, is_alone_sale, is_shipping, goods_desc, add_time, last_update, goods_type, extension_code, rank_integral,user_id)" .
                    "VALUES ('$_POST[goods_name]', '$goods_name_style', '$goods_sn', '$catgory_id', " .
                    "'$brand_id', '$shop_price', '$market_price', '$is_promote','$promote_price', " .
                    "'$promote_start_date', '$promote_end_date', '$goods_img', '$goods_thumb', '$original_img', " .
                    "'$_POST[keywords]', '$_POST[goods_brief]', '$_POST[seller_note]', '$goods_weight', '$goods_number'," .
                    " '$warn_number', '$_POST[integral]', '$goods_virtual','$goods_exp','$goods_newer','$goods_standard',$goods_flaw','$is_local','1','$local_des','$give_integral', '$is_best', '$is_new', '$is_hot', 0, '$is_on_sale', '$is_alone_sale', $is_shipping, " .
                    " '$_POST[goods_desc]', '" . gmtime() . "', '" . gmtime() . "', '$goods_type', '$code', '$rank_integral','$user_id')";
        }
        $db->query($sql);
        $new_id = $db->insert_Id();
          /* 添加选定的城市和地区 */
        if (isset($_POST['local_area']) && is_array($_POST['local_area']))
        {
            foreach ($_POST['local_area'] AS $key => $val)
            {
                $sql = "INSERT INTO ".$ecs->table('goods_region')." (goods_id, region_id) VALUES ('$new_id', '$val')";
                $db->query($sql);
            }
        }
    } else {
        ...
        $sql .= "keywords = '$_POST[keywords]', " .
                ...
                "goods_exp = '$goods_exp', " .
                "goods_newer = '$goods_newer', " .
                "goods_standard = '$goods_standard', " .
                "goods_flaw = '$goods_flaw', " .
                "is_local = '$is_local', " .
                "local_area = '1', " .
                "local_des = '$local_des', " .
                "is_best = '$is_best', " .
        ...
        if (isset($_POST['local_area'])) {
            foreach ($_POST['local_area'] AS $region_id) {
        ...
        /* 清除原有的城市和地区 */
        $db->query("DELETE FROM " . $ecs->table("goods_region") . " WHERE goods_id='$_REQUEST[goods_id]'");

        /* 添加选定的城市和地区 */
        foreach ($selected_regions AS $key => $val) {
            $sql = "INSERT INTO " . $ecs->table('goods_region') . " (goods_id, region_id) VALUES ('$_REQUEST[goods_id]', '$val')";
            $db->query($sql);
        }
    }
    /* 商品编号 */
    ...
    if (isset($_POST['user_rank']) && isset($_POST['user_price'])) {
        handle_member_price($goods_id, $_POST['user_rank'], $_POST['user_price'],$_POST['user_exp']);
    }

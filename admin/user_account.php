...
if ($_REQUEST['act'] == 'alist') {
    ...
    $smarty->assign('ur_here', $_LANG['09_user_exp']);
    $smarty->assign('id', $user_id);
    $smarty->assign('payment_list', $payment);
//    $smarty->assign('action_link',   array('text' => $_LANG['surplus_add'], 'href'=>'user_account.php?act=add'));
...
/* ------------------------------------------------------ */ elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
    ...
    if ($is_paid == 1) {
        ...
        log_account_change($user_id, $amount, 0, 0, 0, 0, 0, $change_desc, $change_type);
    }
...
/* ------------------------------------------------------ */ elseif ($_REQUEST['act'] == 'check') {
    ...
    if ($account['process_type'] == 0) {
        $surplus_info=$_LANG['surplus_info'];
        $surplus_amount=$_LANG['surplus_amount'];
        ...
        $process_type = $_LANG['surplus_type_1'];
         ...
         $surplus_info=$_LANG['exp_info'];
          $surplus_amount=$_LANG['surplus_exp'];
    ...
    $smarty->assign('surplus_info', $surplus_info);
    $smarty->assign('surplus_amount', $surplus_amount);
    $smarty->assign('user_name', $user_name);
    $smarty->assign('id', $id);
    if ($account['process_type'] == 2 || $account['process_type'] == 3) {
        $smarty->assign('action_link', array('text' => $_LANG['09_user_exp'],
            'href' => 'user_account.php?act=alist&' . list_link_postfix()));
    } else {
...
/* ------------------------------------------------------ */ elseif ($_REQUEST['act'] == 'action') {
    ...
    if ($account['is_paid'] == 0) {
        ...
        if ($is_paid == '1' && $account['process_type'] == '1') {
            ...
            //更新会员余额数量
            log_account_change($account['user_id'], $amount, 0, 0, 0, 0, 0, $_LANG['surplus_type_1'], ACT_DRAWING);
        } elseif ($is_paid == '1' && $account['process_type'] == '0') {
            //如果是预付款，并且已完成, 更新此条记录，增加相应的余额
            update_user_account($id, $amount, $admin_note, $is_paid);

            //更新会员余额数量
            log_account_change($account['user_id'], $amount, 0, 0, 0, 0, 0, $_LANG['surplus_type_0'], ACT_SAVING);
        } elseif ($is_paid == '1' && $account['process_type'] == 3) {
            //处理会员购买经验值的申请
            //更新会员经验值数量
            update_user_account($id, $amount, $admin_note, $is_paid);
            log_account_change($account['user_id'], 0, 0, 0, $amount, 0, 0, $_LANG['surplus_type_3'], ACT_SAVING);
            //减少其他会员的经验值，并且为其增加相应的金额
            $cur_amount = $amount;
            $accounts = get_sell_exps();
            foreach ($accounts as $key => $value) {
                if (abs($value['amount']) < $cur_amount) {
                    //更新会员经验值数量
                    log_account_change($value['user_id'], 0, 0, 0, $value['amount'], 0, 0, $_LANG['surplus_type_3'], ACT_SAVING);
                    log_account_change($value['user_id'], abs($value['amount']*$_CFG['exp_sell']),0 , 0,0, 0, 0, $_LANG['surplus_type_3'], ACT_SAVING);
                    $sql = "UPDATE " . $ecs->table('user_account') . " SET " .
                            "admin_user    = '$_SESSION[admin_name]'," .
                            "admin_note    = '系统自动出售经验值', " .
                            "is_paid=1 ".
                            "WHERE id = '$value[id]'";
                    $db->query($sql);
                    $cur_amount+=$value['amount'];
                } else {
                    $amount1=$value['amount']+$cur_amount;
                    log_account_change($value['user_id'], 0, 0, 0, -$cur_amount, 0, 0, $_LANG['surplus_type_3'], ACT_SAVING);
                    log_account_change($value['user_id'], $cur_amount*$_CFG['exp_sell'],0,  0,0, 0, 0, $_LANG['surplus_type_3'], ACT_SAVING);
                    $sql = "UPDATE " . $ecs->table('user_account') . " SET " .
                            "amount    = '$amount1', " .
                            "admin_note    = '部分已经出售' " .
                            " WHERE id = '$value[id]'";
        ...
        } elseif ($is_paid == '1' && $account['process_type'] == 2) {
            
        } 
        elseif ($is_paid == '1' && $account['process_type'] == 5) {
            //处理会员购买经验值兑换金币
            //更新会员经验值数量，出去经验值数
            update_user_account($id, $amount, $admin_note, $is_paid);
            //减少经验值
            log_account_change($account['user_id'], 0, 0, 0, $amount * $_CFG['exp_exchange'], 0, 0, $_LANG['surplus_type_5'], ACT_SAVING);
            //增加金币值
            log_account_change($account['user_id'], 0, 0, abs($amount), 0, 0, 0, $_LANG['surplus_type_5'], ACT_SAVING);
        }
        elseif ($is_paid == '0') {
            /* 否则更新信息 */
            $sql = "UPDATE " . $ecs->table('user_account') . " SET " .
...
function aaccount_list() {
    ...
    if ($result === false) {
        ...
        if ($filter['process_type'] != -1) {
            ...
            $where .= " AND ua.process_type " . db_create_in(array(SURPLUS_TRANSFER, SURPLUS_BUY,SURPLUS_EXCHANGE));
        }
        if ($filter['payment']) {
            $where .= " AND ua.payment = '$filter[payment]' ";
        }
    ...
    $list = $GLOBALS['db']->getAll($sql);
//    var_dump($GLOBALS['_CFG']['exp_sell']);
    foreach ($list AS $key => $value) {
        $list[$key]['surplus_amount'] = abs($value['amount']) . "个(";
        if ($value['process_type'] == '2') {
            $list[$key]['surplus_amount'] .= price_format(abs($value['amount']) * $GLOBALS['_CFG']['exp_sell'], false) . ")";
        } else if ($value['process_type'] == '1'){
            $list[$key]['surplus_amount'] .= price_format(abs($value['amount']) * $GLOBALS['_CFG']['exp_buy'], false) . ")";
        }else{
             $list[$key]['surplus_amount'] .= price_format(abs($value['amount']) * $GLOBALS['_CFG']['exp_exchange'], false) . ")";
        }
        $list[$key]['add_date'] = local_date($GLOBALS['_CFG']['time_format'], $value['add_time']);
        $list[$key]['process_type_name'] = $GLOBALS['_LANG']['surplus_type_' . $value['process_type']];
    }

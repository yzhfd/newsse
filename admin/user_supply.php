...
/*初始化数据交换对象 */
$exc = new exchange($ecs->table("supply_apply"), $db, 'msg_id', 'msg_title');
...
if ($_REQUEST['act'] == 'check')
{
    ...
    {
        /* 允许留言显示 */
        $sql = "UPDATE " .$ecs->table('supply_apply'). " SET msg_status = 1 WHERE msg_id = '$_REQUEST[id]'";
        $db->query($sql);
    ...
    {
        /* 禁止留言显示 */
        $sql = "UPDATE " .$ecs->table('supply_apply'). " SET msg_status = 0 WHERE msg_id = '$_REQUEST[id]'";
        $db->query($sql);
...
if ($_REQUEST['act']=='list_all')
{
    ...
    $smarty->assign('ur_here',      $_LANG['08_supply_list']);
    $smarty->assign('full_page',    1);
...
function msg_list()
{
    ...
    $sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('supply_apply'). " AS f" .
    ...
    $sql = "SELECT f.msg_id, f.user_name, f.msg_title,f.user_phone, f.order_id, f.msg_status, f.msg_time, f.msg_area " .
            "FROM " . $GLOBALS['ecs']->table('supply_apply') . " AS f ".
 ...
 * @return  array
 */
function get_supply_apply_detail($id)
{
    global $ecs, $db, $_CFG;
    ...
    $sql = "SELECT T1.*, T2.msg_id AS reply_id, T2.user_name  AS reply_name, u.email AS reply_email, ".
            ...
            "FROM ".$ecs->table('supply_apply'). " AS T1 ".
            "LEFT JOIN " .$ecs->table('admin_user'). " AS u ON u.user_id='" .$_SESSION['admin_id']. "' ".
            "LEFT JOIN ".$ecs->table('supply_apply'). " AS T2 ON T2.parent_id=T1.msg_id ".
            "WHERE T1.msg_id = '$id'";
    $msg = $db->GetRow($sql);

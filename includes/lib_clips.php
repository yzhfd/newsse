 ...
 */
function get_supply_list($user_id, $user_name, $num, $start, $order_id = 0) {
    /* 获取留言数据 */
    $msg = array();
    $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('supply_apply');
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res)) {
        $reply = array();
        $sql = "SELECT user_name, user_email,user_phone, msg_time, msg_content" .
        ...
        $msg[$rows['msg_id']]['msg_email'] = $rows['msg_email'];
        $msg[$rows['msg_id']]['msg_phone'] = $rows['msg_phone'];
        $msg[$rows['msg_id']]['message_img'] = $rows['message_img'];
        $msg[$rows['msg_id']]['status'] = $rows['msg_status'];
        $msg[$rows['msg_id']]['msg_id'] = $rows['msg_id'];
        switch ($rows['msg_status']) {
            case 0:
                 $msg[$rows['msg_id']]['msg_status']="等待联络中";
                break;
            case 1:
                 $msg[$rows['msg_id']]['msg_status']="客服已联络";
                break;
            case 2:
                 $msg[$rows['msg_id']]['msg_status']="已发货";
                break;
             case 3:
                 $msg[$rows['msg_id']]['msg_status']="正在处理中";
                break;
             case 4:
                 $msg[$rows['msg_id']]['msg_status']="已处理完毕";
                break;
            default:
                break;
        }
 ...
 * @return  string  $order_id       订单ID
 */
function get_local_list($user_id, $num, $start) {
    /* 获取留言数据 */
    $msg = array();
    $sql = "SELECT g.*,u.* FROM " . $GLOBALS['ecs']->table('goods') . " as g left join " . $GLOBALS['ecs']->table('users') . " as u on g.user_id = u.user_id ";

    $sql .= " WHERE g.is_local = 1 AND g.user_id = '$user_id' ORDER BY add_time DESC";
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res)) {
        ...
        $sql = "SELECT a.region_id, r.region_name " .
                "FROM " . $GLOBALS['ecs']->table('goods_region') . " AS a, " . $GLOBALS['ecs']->table('region') . " AS r " .
                "WHERE r.region_id=a.region_id AND a.goods_id='$rows[goods_id]'";
        $res1 = $GLOBALS['db']->query($sql);
        while ($arr = $GLOBALS['db']->fetchRow($res1)) {
            $regions[$arr['region_id']] = $arr['region_name'];
        }
        $msg[$rows['goods_id']]['local_area'] = implode(',', $regions);
        $msg[$rows['goods_id']]['goods_id'] = $rows['goods_id'];
        $msg[$rows['goods_id']]['add_time'] = local_date($GLOBALS['_CFG']['time_format'], $rows['add_time']);
        $msg[$rows['goods_id']]['goods_name'] = nl2br(htmlspecialchars($rows['goods_name']));
        $msg[$rows['goods_id']]['mobile_phone'] = $rows['mobile_phone'];
        $msg[$rows['goods_id']]['local_status'] = $rows['local_status'];
        $msg[$rows['goods_id']]['is_on_sale'] = $rows['is_on_sale'];
        if ($rows['is_on_sale']) {
            if($rows['goods_number']){
            $msg[$rows['goods_id']]['sell_status'] = "在售中";
            }else{
                $msg[$rows['goods_id']]['sell_status'] = "已售完";
            }
            
        } else {
            $msg[$rows['goods_id']]['sell_status'] = "已下架";
        }
    }
    return $msg;
}
...
function add_supply($message) {
    ...
    if ($message['upload']) {
        ...
        $img_name = upload_file($_FILES['message_img'], 'supplyimg');
    ...
    $message['msg_area'] = isset($message['msg_area']) ? intval($message['msg_area']) : 0;
    $sql = "INSERT INTO " . $GLOBALS['ecs']->table('supply_apply') .
            " (msg_id, parent_id, user_id, user_name, user_email, user_phone,msg_title, msg_type, msg_status,  msg_content, msg_time, message_img, order_id, msg_area)" .
            " VALUES (NULL, 0, '$message[user_id]', '$message[user_name]', '$message[user_email]','$message[user_phone]',  " .
            " '$message[msg_title]', '$message[msg_type]', '$status', '$message[msg_content]', '" . gmtime() . "', '$img_name', '$message[order_id]', '$message[msg_area]')";
    $GLOBALS['db']->query($sql);
...
function insert_user_account($surplus, $amount) {
    $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('user_account') .
            ' (user_id, admin_user, amount, add_time, paid_time, admin_note, user_note, process_type, payment, is_paid,to_id)' .
            " VALUES ('$surplus[user_id]', '', '$amount', '" . gmtime() . "', 0, '', '$surplus[user_note]', '$surplus[process_type]', '$surplus[payment]', 0,'$surplus[to_id]')";
    $GLOBALS['db']->query($sql);
...
function get_account_log($user_id, $num, $start) {
    ...
    if ($res) {
        while ($rows = $GLOBALS['db']->fetchRow($res)) {
            ...
            } else if ($rows['process_type'] == 2) {
                $rows['type'] = $GLOBALS['_LANG']['surplus_type_2'];
                $sql = "select user_name from" . $GLOBALS['ecs']->table('users') . " where user_id=" . $rows['to_id'];
                $user_name = $GLOBALS['db']->getOne($sql);
                $rows['amount'] = "转让给" . $user_name . "会员 " . abs($rows['amount']) . " 经验值";
            } else {
 ...
 * @return  array
 */
function get_aaccount_log($user_id, $num, $start) {
    $account_log = array();
    $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('user_account') .
            " WHERE user_id = '$user_id'" .
    ...
    if ($res) {
        while ($rows = $GLOBALS['db']->fetchRow($res)) {
            ...
            /* 会员的操作类型： 买经验值，卖经验值，兑换金币*/
            if ($rows['process_type'] == 2) {
                $rows['type'] = $GLOBALS['_LANG']['surplus_type_3_2'];
                $rows['amount'] = abs($rows['amount']) . "个（" . price_format(abs($rows['amount']) * $GLOBALS['_CFG']['exp_sell'], FALSE) . ")";
            } else if ($rows['process_type'] == 3){
                $rows['type'] = $GLOBALS['_LANG']['surplus_type_3_1'];
                $rows['amount'] = abs($rows['amount']) . "个(" . price_format(abs($rows['amount']) * $GLOBALS['_CFG']['exp_buy'], FALSE) . ")";
            }else{
                $rows['type'] = $GLOBALS['_LANG']['surplus_type_3_3'];
                $rows['amount'] = abs($rows['amount']) . "个(" . price_format(abs($rows['amount']) * $GLOBALS['_CFG']['exp_exchange'], FALSE) . ")";
            }
            ...
            /* 如果是预付款而且还没有付款, 允许付款 */
            if (($rows['is_paid'] == 0) && ($rows['process_type'] == 3 ||$rows['process_type'] == 5)) {
                $rows['handle'] = '<a href="user.php?act=pay&id=' . $rows['id'] . '&pid=' . $pid . '">' . $GLOBALS['_LANG']['pay'] . '</a>';
            }
 ...
 * @return  int
 */
function get_user_exp($user_id) {
    $sql = "SELECT SUM(exp_value) FROM " . $GLOBALS['ecs']->table('account_log') .
            " WHERE user_id = '$user_id'";
...
function get_user_default($user_id) {
    ...
    $sql = "SELECT pay_points, user_money, credit_line, virtual_money,exp_value,last_login, is_validated FROM " . $GLOBALS['ecs']->table('users') . " WHERE user_id = '$user_id'";
    $row = $GLOBALS['db']->getRow($sql);
    $info = array();
    ...
    $info['formated_virtual_money'] = price_format($info['virtual_money'], false);
    $info['exp_value'] = $row['exp_value'];

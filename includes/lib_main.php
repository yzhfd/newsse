...
function assign_ur_here($cat = 0, $str = '')
{
    ...
    if ($filename != 'index')
    {
        ...
        else
        {
            ...
            {
                $page_title = $GLOBALS['_LANG']['local'] . '_' . $page_title;
                $args       = array('wsid' => '0');
                $ur_here   .= ' <code>&gt;</code> <a href="local.php">' .
                                $GLOBALS['_LANG']['local'] . '</a>';
            }
        }
    }
                        ...
                        $keywords = '', $brand = 0, $price_min = 0, $price_max = 0, $display_type = 'list', $filter_attr='', $url_format='', $sch_array='')
{
    ...
    switch ($app)
    {
        ...
        case 'exchange':
            ...
            break;
        case 'local':
            $uri_args = array( 'pid' => $brand, 'cid'=>$price_min, 'did'=>$price_max, 'sort' => $sort, 'order' => $order, 'display' => $display_type);
            break;
    }
    /* 分页样式 */

        ...
        if ($pay_log && $pay_log['is_paid'] == 0)
        {
            ...
            if ($pay_log['order_type'] == PAY_ORDER)
            {
                ...
                if (!empty($virtual_goods))
                {
                    ...
                    if ($order['shipping_id'] == -1)
                    {
                        ...
                        log_account_change($order['user_id'], 0, 0,0,0, intval($integral['rank_points']), intval($integral['custom_points']), sprintf($GLOBALS['_LANG']['order_gift_integral'], $order['order_sn']));
                    }
                }
            ...
            elseif ($pay_log['order_type'] == PAY_SURPLUS)
            {
                ...
                if(empty($res_id))
                {
                    ...
                    log_account_change($arr['user_id'], $arr['amount'], 0,0,0, 0, 0, $_LANG['surplus_type_0'], ACT_SAVING);
                }
            }
        }
        else

 ...
 * @return  void
 */
function log_account_change($user_id, $user_money = 0, $frozen_money = 0,$virtual_money=0,$exp_value=0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER)
{
    /* 插入帐户变动记录 */
    $account_log = array(
        'user_id'       => $user_id,
...
//获取当前网站在卖的经验值总数
function get_total_exp(){
    $sql="select SUM(amount) FROM ".$GLOBALS['ecs']->table('user_account')." where process_type=2 and admin_user =''";
    $amount=$GLOBALS['db']->getOne($sql);
    return abs($amount);
}
//获取当前网站在卖的经验值记录
function get_sell_exps(){
    $sql="select * FROM ".$GLOBALS['ecs']->table('user_account')." where process_type=2 and admin_user ='' order by add_time";
    $amounts=$GLOBALS['db']->getAll($sql);
    return $amounts;
}
?>
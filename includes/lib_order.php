...
function order_info($order_id, $order_sn = '') {
    ...
    if ($order) {
        ...
        $order['formated_money'] = price_format($order['money'], false);
...
function order_fee($order, $goods, $consignee) {
    $sql = "SELECT min(flow_no) FROM " . $GLOBALS['ecs']->table('cart') . " WHERE session_id ='" . SESS_ID."'";
    $flow_no = $GLOBALS['db']->getOne($sql);
    /* 初始化订单的扩展code */
    ...
    if ($order['shipping_id'] > 0 && $total['real_goods_count'] > 0) {
        ...
        if (!empty($shipping_info)) {
            ...
            $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('cart') . " WHERE  `session_id` = '" . SESS_ID . "' AND `extension_code` != 'package_buy' AND `is_shipping` = 0 AND flow_no=" . $flow_no;
    ...
    /* 金币 */
    $order['money'] = $order['money'] > 0 ? $order['money'] * $GLOBALS['_CFG']['money_per'] : 0;
    if ($total['amount'] > 0) {
        if (isset($order['money']) && $order['money'] > $total['amount']) {
            $order['money'] = $total['amount'];
            $total['amount'] = 0;
        } else {
            $total['amount'] -= floatval($order['money']);
        }
    } else {
        $order['money'] = 0;
        $total['amount'] = 0;
    }
    $total['money'] = $order['money'];
    $total['money_formated'] = price_format($order['money'], false);
    ...
    if ($order['extension_code'] == 'exchange_goods') {
        $sql = 'SELECT SUM(eg.exchange_integral) ' .
                ...
                '  AND c.is_gift = 0 AND c.goods_id > 0 AND c.flow_no='.$flow_no .
...
function cart_goods($type = CART_GENERAL_GOODS) {
    ...
    $sql = "SELECT rec_id, user_id, goods_id, goods_name, goods_sn, goods_number, " .
            ...
            "AND rec_type = '$type' AND flow_no=" . $flow_no;
    ...
    foreach ($arr as $key => $value) {
        ...
        $arr[$key]['is_local'] = $goods_info['is_local'];
        if ($goods_info['is_local']) {
            $regions = array();

            $sql = "SELECT a.region_id, r.region_name " .
                    "FROM " . $GLOBALS['ecs']->table('goods_region') . " AS a, " . $GLOBALS['ecs']->table('region') . " AS r " .
                    "WHERE r.region_id=a.region_id AND a.goods_id='$goods_info[goods_id]'";
            $res = $GLOBALS['db']->query($sql);
            while ($arr1 = $GLOBALS['db']->fetchRow($res)) {
                $regions[$arr1['region_id']] = $arr1['region_name'];
            }
            $arr[$key]['local_area'] = implode(',', $regions);
        }
...
function cart_weight_price($type = CART_GENERAL_GOODS) {
    ...
    $flow_no = $GLOBALS['db']->getOne($sql);
    /* 计算超值礼包内商品的相关配送参数 */
    $sql = 'SELECT goods_id, goods_number, goods_price FROM ' . $GLOBALS['ecs']->table('cart') . " WHERE extension_code = 'package_buy' AND session_id = '" . SESS_ID . "' and flow_no=" . $flow_no;
    ...
    $sql = 'SELECT SUM(g.goods_weight * c.goods_number) AS weight, ' .
            ...
            "AND rec_type = '$type' AND g.is_shipping = 0 AND c.extension_code != 'package_buy' and c.flow_no=" . $flow_no;
    $row = $GLOBALS['db']->getRow($sql);
...
function check_local_cart($goods_id) {
    $sql = "select count(*) as num from " . $GLOBALS['ecs']->table('cart') . " where is_local=1 and goods_id = $goods_id";
    $first = $GLOBALS['db']->getOne($sql);
    if ($first) {
        return false;
    }
    $sql = "select count(*) as num from " . $GLOBALS['ecs']->table('cart') . " where is_local=1 and goods_id != $goods_id";
    ...
    if ($first) {
        return true;
    } else if (check_empty_cart()) {
        $sql = "select is_local from " . $GLOBALS['ecs']->table('goods') . " where goods_id=" . $goods_id;
...
function addto_cart($goods_id, $num = 1, $spec = array(), $parent = 0) {
    ...
    /* 取得商品信息 */
    $sql = "SELECT g.goods_name, g.goods_sn, g.is_on_sale, g.is_real, g.is_local," .
    ...
    if ($num > 0) {
        ...
        if ($row) { //如果购物车已经有此物品，则更新
            ...
            if (empty($carts)) {
                ...
                $sql = "SELECT max(flow_no) FROM " . $GLOBALS['ecs']->table('cart') . " WHERE session_id='" . SESS_ID . "'";
                $flow_no = $GLOBALS['db']->getOne($sql);
                $sql = "select is_local from " . $GLOBALS['ecs']->table('goods') . " where goods_id=" . $goods_id;
                $is_local = $GLOBALS['db']->getOne($sql);
                if ($is_local) {//如果是同城商品，那么flow_no直接加1
                    $parent['flow_no'] = $flow_no + 1;
                } else {//如果是非同城商品，就和其他的非同城商品的flow_no一样。
//                    $parent['flow_no'] = $flow_no;
                    $sql = "SELECT flow_no FROM " . $GLOBALS['ecs']->table('cart') . " WHERE session_id='" . SESS_ID . "' and is_local!=1";
                    $flow_no1 = $GLOBALS['db']->getOne($sql);
                    if ($flow_no1) {
                        $parent['flow_no'] = $flow_no1;
                    } else {
                        $parent['flow_no'] = $flow_no + 1;
                    }
                }
...
function clear_cart($type = CART_GENERAL_GOODS) {
    ...
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('cart') .
            " WHERE session_id = '" . SESS_ID . "' AND rec_type = '$type' and flow_no=" . $flow_no;
    $GLOBALS['db']->query($sql);
}
...
//        }
        ...
        $user['formated_virtual_money'] = price_format($user['virtual_money'], false);
...
function order_refund($order, $refund_type, $refund_note, $refund_amount = 0) {
    ...
    /* 处理退款 */
    if (1 == $refund_type) {
        log_account_change($user_id, $amount, 0, 0, 0, 0, 0, $change_desc);
...
function get_cart_goods() {
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        $goods_list[$row['flow_no']]["goods_price"]+=$row['goods_price'] * $row['goods_number'];
        $goods_list[$row['flow_no']]['market_price'] += $row['market_price'] * $row['goods_number'];
...
//        $goods_list[] = $row;
        $goods_list[$row['flow_no']]['goods_list'][]=$row;
     
    }
    foreach($goods_list as $k=>$v){
        $goods_list[$k]['goods_price']=price_format($v['goods_price'], false);
        $goods_list[$k]['market_price']=price_format($v['market_price'], false);
    }
...
function get_total_bonus() {
    ...
    $sql = "SELECT SUM(c.goods_number * t.type_money)" .
            ...
            "AND c.rec_type = '" . CART_GENERAL_GOODS . "' AND c.flow_no=" . $flow_no;
    ...
    $sql = "SELECT SUM(goods_price * goods_number) " .
            ...
            " WHERE session_id = '" . SESS_ID . "' " .
            " AND is_gift = 0 " .
            " AND rec_type = '" . CART_GENERAL_GOODS . "' AND flow_no=" . $flow_no;
...
function compute_discount() {
    ...
    $sql = "SELECT c.goods_id, c.goods_price * c.goods_number AS subtotal, g.cat_id, g.brand_id " .
            ...
            "AND c.parent_id = 0 " .
            "AND c.is_gift = 0 " .
            "AND rec_type = '" . CART_GENERAL_GOODS . "' AND c.flow_no=" . $flow_no;
...
function get_give_integral() {
    ...
    $sql = "SELECT SUM(c.goods_number * IF(g.give_integral > -1, g.give_integral, c.goods_price))" .
            ...
            "AND c.parent_id = 0 " .
            "AND c.rec_type = 0 " .
            "AND c.is_gift = 0 AND c.flow_no=" . $flow_no;

...
function changeMoney(val){
  ...
  Ajax.call('flow.php?step=change_money', 'money=' + val, changeMoneyResponse, 'GET', 'JSON');
}
/* *
 * 改变金币回调函数
 */
function changeMoneyResponse(obj)
{
  if (obj.error)
  {
    try
    {
      document.getElementById("ECS_MONEY_NOTICE").innerHTML = obj.error;
      document.getElementById('ECS_MONEY').value = '0';
      document.getElementById('ECS_MONEY').focus();
    }
    catch (ex) { }
  }
  else
  {
    try
    {
      document.getElementById("ECS_MONEY_NOTICE").innerHTML = '';
    }
    catch (ex) { }
    orderSelectedResponse(obj.content);
  }
